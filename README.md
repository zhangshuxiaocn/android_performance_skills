...
- **三类递归分析**: 对每个耗时节点按状态分类递归（①Running→频率/内容+CPU核心频点 ②R/R+→抢占者分析 ③Sleep/D→唤醒链追踪），最多 5 层，直到所有耗时都有终端根因
- **统一根因链路图**: 所有分析结果（Binder 服务端分解、锁竞争递归、调度优先级、优先级反转）融入同一棵根因树，不割裂展示
- **耗时可对账**: 根因链路图中每层子节点耗时之和约等于父节点，未覆盖部分按线程状态（Running/Sleep/D-IO）分类补齐，确保每层分解可向上对账
- **Sleep 强制递归**: 根因链路图中任何节点（包括未覆盖部分、已命名 slice、阶段间隙）的 Sleep >10ms，都必须通过唤醒链追踪到终端根因，不允许停留在"Sleep XXms"
- **每阶段精确状态矩阵**: 对每个顶层 slice（bindApplication、doFrame、clientTransactionExecuted 等）查询精确的线程状态分布（Running/Sleep/D-IO/R+R/D non-IO），所有数值来自 SQL 查询，不做估算
- **根因时间分解**: 核心结论中用两张表格量化根因贡献 — 全局根因时间分解（CPU Running / D-IO / Sleep 含 CFS/RT 拆分 / R/R+ / D non-IO）+ 按阶段拆分的精确状态矩阵
- **结构化报告**: 5 节报告，从概览到根因链路图与核心结论
...
5. **问题总结与根因分析**
   - 5.1 根因链路图 — 统一因果树，三类递归分析逐层展开到终端根因
   - 5.2 核心结论与问题清单 — 根因时间分解表 + 核心问题清单

根因链路图是报告的核心，将所有分析结果统一呈现:
- 基于 trace 中 slice 的实际父子关系逐层递归展开，每个顶层原因递归到终端原因（最深 5 层）
- 三类递归分析: ①Running→slice 内容 + CPU 核心/频点 ②R/R+→抢占者列表 ③Sleep/D→唤醒链追踪或 blocked_function
- Binder 深度分析（调用链、AIDL 接口、服务端 slice 树、服务端线程状态）融入对应的 binder transaction 分支
- 锁竞争分析（持有者线程状态、持有者执行内容）融入对应的 monitor contention 分支
- 调度优先级分析（RT boost、异常抢占）和依赖链优先级反转（CFS 唤醒者放大效应）融入对应节点

核心结论用**根因时间分解表**量化各类根因贡献:
- CPU Running / I/O 加载（D-IO）/ 阻塞等待（Sleep，含 CFS/RT 依赖拆分）/ 调度等待（R/R+）/ 内核操作（D non-IO）
- 各项之和约等于启动总耗时，CFS/RT 拆分来自唤醒链优先级分类
