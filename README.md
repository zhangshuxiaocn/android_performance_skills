...
5. **问题总结与根因分析** — 核心结论、根因链路图、核心问题清单

根因链路图是报告的核心，将所有分析结果统一呈现:
- 基于 trace 中 slice 的实际父子关系逐层递归展开，每个顶层原因递归到终端原因（最深 10 层）
- Binder 深度分析（调用链、AIDL 接口、服务端 slice 树、服务端线程状态）融入对应的 binder transaction 分支
- 锁竞争分析（持有者线程状态、持有者执行内容）融入对应的 monitor contention 分支
- 调度优先级分析（RT boost、异常抢占）和依赖链优先级反转（CFS 唤醒者放大效应）作为独立顶层原因
- 递归依赖链追踪（锁阻塞 >50ms 或 CFS 唤醒阻塞 >30ms）逐层展开阻塞者状态直到终端根因
