...
基于 [Perfetto](https://perfetto.dev/) trace 文件，自动执行 17 个 SQL 查询，并根据结果动态触发递归依赖链追踪，输出 6 节结构化中文分析报告。
...
- **结构化报告**: 6 节报告，从概览到根因链路图（含递归展开的多层依赖链）
...
5. **调度优先级分析** — RT boost、异常抢占、依赖链优先级反转
6. **问题总结与根因分析** — 核心结论、根因链路图（含递归依赖链展开）、问题清单
